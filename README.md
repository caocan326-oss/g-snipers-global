...
**本 PR 不实现、不预留 Google Ads / MCC / OAuth / 广告系列同步，也没有「连接广告账户」按钮（包括会失败的死按钮）。**

原因：目前没有可用的 Google Ads 测试账号，任何 Ads 形态都无法验收。等测试账号到位后再单独开切片。启动应用**不需要**任何 `GOOGLE_ADS_*` 环境变量。本地用 seed 演示数据即可跑通洞察与 SEO。
...
- **人工确认门**：`POST /api/seo-pages/{id}/mark-ready` 必须 `confirmed: true`，否则拒绝。本切片不会自动发布到客户站点。
...
- **SEM / Google Ads / Meta**：OAuth、MCC、广告账户连接、广告系列同步、改预算、自动投放（等有测试账号再做）
- GEO 监控、Share of Voice、新媒体中心、ROI 总看板
- 关键词研究工具、站点体检、mock 出来的无用模块大盘
- 任何虚构的 Google / SERP / Ads API。大纲与正文是**本地模板**，供客户经理改稿；需求信号是工作区内存量（`source=manual`）
