...
可选（留空 = 未配置）：`LLM_API_KEY`（及 `LLM_BASE_URL` / `LLM_MODEL`）、`GSC_OAUTH_*`、`GOOGLE_RELAY_URL` / `GOOGLE_RELAY_KEY`（北京 GSC 同步和 PageSpeed 走 Cloudflare Worker，见 `deploy/google-relay-worker/README.md`）、`PAGESPEED_API_KEY`、`BING_WEBMASTER_API_KEY`、`INDEXNOW_KEY`、`DISTRIBUTION_*_API_KEY`。中转配上后测速不再走 17CE。

Bright Data Browser API 用于 JS 空壳页的二次渲染复查，可选配置：`BRIGHTDATA_BROWSER_WS`、`ONSITE_RENDER_JS_ENABLED`、`ONSITE_RENDER_TIMEOUT_MS`。

Bright Data SERP API 用于目标国家/关键词下的 Google 搜索结果观察。请求走 `https://api.brightdata.com/request`，必须带 SERP 区名和 `https://www.google.com/search?q=...&gl=...&hl=...&brd_json=1`。可选配置：`BRIGHTDATA_DATASET_API_KEY`、`BRIGHTDATA_SERP_ZONE`。Browser API、Dataset scrape 和 SERP API 是三条通道，不要把浏览器区名填进 SERP 区。不要把真实连接串、Bearer Key 或密码提交进仓库。
