...
最后更新：2026-08-27 19:18（公司。门锁拒收先于太短，发版补测。）。
...
2. 不过（`c84d173`）：贴「智能门锁适合租客安装」拒了、包没落，红字仍是太短，没有「门锁」。已改成先认门锁，随本轮发版。补测见当前状态。
...
| 分支 | `main` |
| 提交 | 产品 `1c2ab88`。交接随本提交。 |
| 已 push origin / upstream | 是（随本轮一起推） |
| 已发版生产 | **是。** `sync-from-local.ps1 -Rebuild`。门锁拒收先于太短。 |
| 接口实测 | 现网 1/3/4 过。第 2 条待硬刷新补测。 |
| 记下没挡 | SNIPERS 未批草稿先留着。August Home / Level Lock / Qrio 还在诊断目标。 |
| 仍做不到 / 不要做 | 网址过深本周做不到。llms.txt 做不到。贴上了不等于被提到。不要编问句、不要代改、不要再测客户网站、不要登 `139.129.99.194`。 |
| 下一台先做 | 硬刷新补测：贴「智能门锁适合租客安装」，红字含门锁、没有太短。不批准。 |
