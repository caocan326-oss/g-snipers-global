...
最后更新：2026-08-23 13:09（家里。产品 `363b37b`。官网核验收口见 §4.9。GEO 工单只跟海外源见 §4.9。错误拆分见 §4.8。上线地址见 §4.6 `01d8b3b`。）。
...
人工验收（生产绿联）：英文 100W 问跑一轮 → 有 ugreen.com 则打开并核对通过 → 京东勾应失败 → 客户说明文案对得上 → 待处理项只跟海外源 → 上线 URL 后再测只记变化。
...
| 分支 | `main` |
| 提交 | 产品 `363b37b`（官网核验收口，见 §4.9）。工单只跟海外源 `e1d2435`。分类与错误拆分 `e93dbf6`（§4.8 / §4.9）。上线地址 `01d8b3b`（见 §4.6）。用量与空态 `432fd6a`（见 §4.7）。绿联身份见 §4.4。三处对齐见 §4.5。客户改法见 §4.6。百炼见 §4.3。运维：§4.1 / §4.2。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `363b37b` 已 `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。Postgres 仍不映射 5432。 |
| 接口实测 | `test_geo_verify_owned` 2 通过；`test_geo_ticket_kinds` 7 通过；`test_geo_citations` 4 通过；`test_http_errors` 通过。 |
| 未完成 | 生产绿联那句英文问的人工验收（打开 ugreen.com 核对、客户说明、再测）还没走完。没有单独的绿联客户经理号。IndexNow / GSC 仍要配钥匙。不要公开注册。不要自动群发。 |
| 下一台先做 | 公司先 `git pull origin main`。强制刷新后按 §4.9 人工验收清单走绿联英文问。不要两边同时改。 |
