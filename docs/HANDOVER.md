...
最后更新：2026-08-27 22:32（家里。`82dd632` 门锁竞品过滤已发版，待 T2/T3。）。
...
- T2 / T3 **待发版后再测**：保存后会不会再冒出来；故意填 August Home 会不会被收下。
...
| 分支 | `main` |
| 产品提交 | `82dd632` — 活客户诊断目标清掉门锁竞品。见 §4.49。 |
| 文档提交 | 本提交：发版记录 + 人测 T1/T4。 |
| 已 push origin / upstream | 是（随本提交一起推） |
| 已发版生产 | **是。** `82dd632` 已 `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。 |
| 本机工作区 | 提交并推完后应干净。 |
| 接口实测 | T1/T4 过（发版前库已空）。T2/T3 **待硬刷新再测**。 |
...
门锁竞品过滤已发（`82dd632`）。先硬刷新测 T2/T3，见 §4.49。还没排：批准 Fact Pack（必须客户确认过的英文）、清未批那两份草稿、Monitor SaaS、代改官网。用户说做哪件再做哪件。
