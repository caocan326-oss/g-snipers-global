...
最后更新：2026-08-27（cloud agent。§4.32–4.36 / §4.45 已写进仓库，生产未发这轮。）。
...
现在经理点「记受模板限制」：不关单、不代改。问题板还在，处理清单进受阻。这周改三处和客户说明跳过这条，补下一页紧急/优先。可点「取消受模板限制」加回去。不要用「本轮不改」记这件事。不会自动给现网 `cat_id/3` 打上，要点一下。
...
经理自己点「打开核对」：浏览器打开该页，工作台只记看过。不代改。**不会自己勾完，也不会从这三处拿掉。** 抓页若把状态写成已解决，要拨回去。拿掉过的可「放回刚拿掉的一页」。**不是等客户改完才能交付。** 客户改不改官网不挡工作台。`cat_id/3` JSON-LD 人眼 10:32 已过；它仍是这周第 1 张，不要因为工作台自己勾完就换页。
...
登记询盘时可选本户已记买家问句；已有的条也能后挂。挂上不是证明 AI 提到了我们。没有原句就空着，不要编。客户说明中英都会写出挂了哪几句。迁移 `026`。不开客户站、不再抽。

### 4.45 总览「今天先做」+ 门锁演示询盘不要回到 SNIPERS

`b2c9bc1` 已把 Fact Pack 排到工作台下一步第一位，并在打开 SNIPERS 时清掉门锁演示询盘。本 PR 把总览第一屏 / 第三块 / 优先队列改成这周项，总览可手录原句，打开核对在总览也能点。
...
| 提交 | 总览这周项 + `this_week_open` + 总览手录原句 / 打开核对。交接 §4.32–4.36、§4.45 随本提交。 |
