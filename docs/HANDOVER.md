...
最后更新：2026-08-23 02:02（家里。产品 `e93dbf6`。错误拆分与控制台横幅见 §4.8。购物页不算官网见 §4.9。上线地址见 §4.6 `01d8b3b`。绿联身份见 §4.4。同一问三处对齐见 §4.5。客户改法见 §4.6。人工复测修补见 §4.7。）。
...
`e93dbf6` 起：每条链接按规则分成官网、购物页、其它。购物页永不算「给出官网」。客户如果把店登记成官网，仍认官网。旧抽查不用重跑，读出来时按同一套规则拆。批次卡拆成：疑似官网 / 购物页（不算官网） / 其它外来网址。工单话术和复测对比还没吃这三类。
...
| 分支 | `main` |
| 提交 | 产品 `e93dbf6`（连不上与 500 拆开、控制台横幅、购物页不算官网，见 §4.8 / §4.9）。上线地址 `01d8b3b`（见 §4.6）。用量与空态 `432fd6a`（见 §4.7）。绿联身份见 §4.4。三处对齐见 §4.5。客户改法见 §4.6。百炼见 §4.3。运维：§4.1 / §4.2。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `e93dbf6` 已 `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。Postgres 仍不映射 5432。 |
| 接口实测 | `test_geo_citations` 4 通过；`test_geo_sample_run_freezes_manual_observations_as_evidence`、`test_geo_bocha_and_bailian_provider_adapters` 通过。 |
| 未完成 | GEO 工单和复测还没按购物页改话术。没有单独的绿联客户经理号，这次用 `admin@weiyids.com`。IndexNow / GSC 仍要配钥匙。不要公开注册。不要自动群发。 |
| 下一台先做 | 公司先 `git pull origin main`。强制刷新：后端挂了应留在控制台看页顶横幅，不要被踢登录。GEO 批次卡里京东/Amazon 应标「购物页（不算官网）」。不要两边同时改。 |
