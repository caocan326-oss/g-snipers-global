...
最后更新：2026-08-22 18:52（公司。产品 `46ac4e7`。绿联身份见 §4.4。同一问三处对齐见 §4.5。）。
...
| 分支 | `main` |
| 提交 | 产品 `46ac4e7`（同一问写全，标题跟着这一轮抽查改）。绿联身份见 §4.4。三处对齐见 §4.5。百炼见 §4.3。运维：§4.1 / §4.2。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。Postgres 仍不映射 5432。 |
| 接口实测 | `test_geo` / `test_customer_brief` / `test_usage` 40 通过。发完 health 200。本机 / origin / upstream / 生产均为 `46ac4e7`。 |
| 未完成 | 博查常给中文电商、没有 ugreen.com（源质量）。工作台「人工上线」不是官网已改。用量 0/24 要对照成功调用后再看。IndexNow / GSC 仍要配钥匙。不要公开注册。不要自动群发。 |
| 下一台先做 | 家里先 `git pull origin main`。刷新清单和客户说明即可核同一条 100W USB-C 问，不必再抽查。不要两边同时改。家里 `localtest.db` 不要提交。 |
