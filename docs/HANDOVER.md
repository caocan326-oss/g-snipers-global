...
| 分支 | `main` |
| 提交 | 产品 `6025d78`（短稿卡内「不代改」，§4.11）。办事卡 `c2ea32a`（§4.10）。人测见 §4.10。 |
| 已 push origin / upstream | 是（同最新 docs 提交）。 |
| 已发版生产 | **是。** `6025d78` 已 `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。Postgres 仍不映射 5432。 |
| 接口实测 | 本地短稿相关单测通过。生产绿联 A/B/D 过；C2 初测不过后已发 `6025d78`，待硬刷新复看「不代改」。A8 未测。 |
| 未完成 | **复测 C2**：agents.md / 100W GEO 短稿末行有「不代改」。A8 同一问再测（有额度再测）。绿联产品页仍须**客户改站**后再测。站外不进交付。IndexNow / GSC 仍要配钥匙。 |
| 下一台先做 | ① 硬刷新复测 §4.11。② 公司机若有 clone：`git pull origin main`。③ 够用则下一刀才是站外「一篇稿 + 回填链接」接到 GEO；不够则先改 GEO/站内文案。不要两边同时改。本机无 `E:\G-snipers海外版`。 |
