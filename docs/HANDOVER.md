...
最后更新：2026-08-24 13:41（公司。产品 `85ff828`。站外可复制给客户、可看报文、可记自备接口。不代发。）。
...
| 分支 | `main` |
| 提交 | 产品 `85ff828`（§4.18 复制给客户 / 报文 / 自备接口）。§4.15–4.17 人测过。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `85ff828` 已 `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。Postgres 仍不映射 5432。 |
