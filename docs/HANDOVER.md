...
最后更新：2026-08-27 13:05（公司。清 SNIPERS 门锁 llms.txt 草稿。）。
...
经理不要再催后台改 JSON-LD。能改的说明已经改完。标记要动模板，那是他们网站的事，我们不代改。工作台这条可记「受模板限制」，不要当成客户没理。下周换能改的页，或等他们网站的人改模板后再核。不要把 102 条整包丢回去。
...
**卡在他们网站权限，不是我们产品。** 后台文章分类只能改名称 / 关键字 / 描述，动不了 JSON-LD。这段写死在主题文件里，要 `139.129.99.194` 上 `/template/pc/snipers/` 的文件权限。推广说他们没有这权限。改法（上面「只许补」那几项）已写下，交给**有这台机文件权限的人**去改主题。`139.129.99.194` 是客户站，不是我们生产 `39.97.52.149`。我们不登那台机、不代改主题。工作台这条记「受模板限制」。不挡往下开发。下周换他们后台能改的页，或等有权限的人改完再说一声，经理再只打开这一页核标记。
