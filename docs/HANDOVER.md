...
最后更新：2026-08-27 12:10（公司。打开核对 + 引用资产闭环发版。）。
...
| 分支 | `main` |
| 提交 | `62fb19e` 作战室：真问句时间序列 + 可引用资产。问题板不是这周要做完。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `sync-from-local.ps1 -Rebuild`。三处 `62fb19e`。迁移 `024` 已跑。`DEMO_LOGIN_ENABLED` 仍关。 |
| 接口实测 | §4.28、§4.29 人测过。`cat_id/3` 标记 10:32 过。打开核对不是发版挡板。 |
| 未完成 | 引用资产闭环 §4.37 未发版。打开核对修复未发版。常驻复测这周不发。没有原句则 GEO 空着。 |
| 下一台先做 | 发版后总览点「放回刚拿掉的一页」，`cat_id/3` 应回第 1 张且已发给客户。再点「打开核对」应新开官网、三处不动。不代改、不编问句。 |
