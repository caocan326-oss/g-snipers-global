...
最后更新：2026-08-27 09:50（公司。§4.28 工作台可记受模板限制。已 Rebuild 发版。SNIPERS。）。
...
| 分支 | `main` |
| 提交 | 产品：工作台可记受模板限制，这周三处换下一页。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。 |
| 接口实测 | 绿联两轮抽查已踩实。`cat_id/3` 现网标记仍只有 name+url。记下受模板限制要人测点一下。 |
| 未完成 | 经理在站内对 `cat_id/3` 那条点「记受模板限制」。等有主题权限的人改完再说一声，再只核这一页标记。 |
| 下一台先做 | 家里先 `git pull origin main`。硬刷新后人测 §4.28。不登 `139.129.99.194`、不代改、不编问句、不抽第三轮绿联。 |
