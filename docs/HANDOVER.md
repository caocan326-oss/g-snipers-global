...
最后更新：2026-08-26 19:05（公司。§4.25 T1–T4 人测过。现网当前站 SNIPERS。）。
...
| 分支 | `main` |
| 提交 | 产品 `6b5532c`（§4.25 T1–T4 人测过）。现网当前站 **SNIPERS**。绿联已归档。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `6b5532c` 已 `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。 |
| 接口实测 | 人测 T1–T4 过。空着采用 0 条。 |
| 未完成 | 真发须客户自己的号。SNIPERS 没有可填的 LinkedIn / Facebook 公司页。要绿联时从历史网站恢复。 |
| 下一台先做 | 家里先 `git pull origin main`。采用已记原句和门锁残留都已收口，不要重开。空 GEO 不要编问句。下一刀不是代发。不要两边同时改。家里 `localtest.db` 不要提交。 |
