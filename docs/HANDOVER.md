...
最后更新：2026-08-26 19:52（公司。§4.26 `cat_id/3` 现网标记仍只有 name+url，公司和产品还没补上。认证空着是对的。）。
...
| 分支 | `main` |
| 提交 | 产品 `6b5532c`。`cat_id/3` 现网标记仍只有 name+url，已交推广，认证空着是对的。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** 产品未改。`DEMO_LOGIN_ENABLED` 仍关。 |
| 接口实测 | 绿联两轮抽查已踩实。三页说明对上。 |
| 未完成 | 现网还没补上。等推广改完再核公司/产品；认证须空。不要厦门成都、不要编规格。 |
| 下一台先做 | 家里先 `git pull origin main`。推广改完只核标记，不代改、不编认证。不要为额度再抽绿联。下一刀不是代发。 |
