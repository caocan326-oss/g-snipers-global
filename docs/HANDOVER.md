...
最后更新：2026-08-27 19:22（公司收工。本机开发暂停。回家 `D:\workspace\G-snipers海外版` 继续。）。
...
2. 不过（`c84d173`）：贴「智能门锁适合租客安装」拒了、包没落，红字仍是太短，没有「门锁」。已发 `1c2ab88`：门锁拒收先于太短。公司收工时人还没回报补测。回家硬刷新再测，见 §6。
3. 过：页上已有客户英文收成草稿。官网 `https://www.snipers.com.cn`。认证空。GEO 仍挡对外稿。
4. 过：总览「核对并批准 Fact Pack / 草稿待批」。没点批准。
5. 空着（不批）。SNIPERS 多了一份未批草稿，是客户英文，先留着。不批准不出对外稿。
...
| --- | --- |
| 日期 | 2026-08-27 19:22 |
| 最后一台 | **公司收工** `E:\G-snipers海外版`。今天这台不再改代码。 |
| 下一台 | **家里** `D:\workspace\G-snipers海外版` |
| 分支 | `main` |
| 产品提交 | `1c2ab88` — 门锁拒收先于太短 |
| 文档提交 | 本提交（公司收工交接） |
| 已 push origin / upstream | 是（随本提交一起推） |
| 已发版生产 | **是。** `sync-from-local.ps1 -Rebuild`。health 200。线上就是 `1c2ab88`。 |
| 本机工作区 | 公司这台提交并推完后应干净。不要在公司再开新改动。 |
| 接口实测 | `c84d173` 走查 1/3/4 过、5 空着不批。第 2 条（门锁红字）**还没人回报过/不过**。 |
| 记下没挡 | SNIPERS 有一份**未批** Fact Pack 草稿（客户英文，19:16 走查留下）。August Home / Level Lock / Qrio 还在诊断目标。 |
...
`git status` 必须干净。`git log -1` 应能看到本提交（公司收工交接）。不要 `git pull upstream`。

然后读本文 §6 和 `docs/PRODUCTION.md` 第 7、9 节。做完 §2 那 7 步才能改代码。

**今晚先不要开新功能。** 先在生产硬刷新补测门锁拒收文案（下面 6.2）。测完回报过/不过，再记交接。用户没说提交/发版不要推。

### 6.2 家里先补测（只这一条，现网）

站点：https://www.weiyids.com（硬刷新，不要当本机 Docker）。页头必须是 **当前客户：SNIPERS** / `https://www.snipers.com.cn`。

1. 打开 **站外 → 对外稿**（`/offsite?tab=content`）。
2. 原文原样贴：`智能门锁适合租客安装`
3. 点「从资料收成草稿」。

| | 过 | 不过 |
| --- | --- | --- |
| 红字 | 含「门锁」（现文是「这是门锁演示资料，不能记到这个客户。不要编。」） | 只说「太短」，没有「门锁」 |
| 包 | 不新落一份包；原来那份未批草稿还在 | 又多出一份门锁草稿 |

不要：打开客户官网、抽查、烧配额、批准 Fact Pack、清未批草稿、切测试站、手加门锁问句。
...
### 6.4 补测过了之后（家里再问用户，不要自己开）

还没排进今晚的：批准 Fact Pack（必须客户确认过的英文）、清未批草稿、清诊断目标里的 August/Level/Qrio、Monitor SaaS、代改官网。用户说做哪件再做哪件。
