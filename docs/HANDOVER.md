...
最后更新：2026-08-24 12:36（公司。产品 `e00f8d6`。旧 451 行也带「登记≠我们代发」。§4.16 角标渠道过。）。
...
| 分支 | `main` |
| 提交 | 产品 `e00f8d6`（旧 451 行也带「登记≠我们代发」，§4.17）。§4.16 角标渠道过。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `e00f8d6` 已 `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。Postgres 仍不映射 5432。 |
| 接口实测 | `test_distribution` 8 通过。执行记录待硬刷新复看核验那一句。 |
| 未完成 | **复看执行记录核验行**：`URL 返回 HTTP 451，暂未通过存活核验。登记≠我们代发。` 真发须客户自己的号。A8 有额度再测。 |
| 下一台先做 | 家里先 `git pull origin main`。硬刷新站外执行记录核验行。不要两边同时改。家里 `localtest.db` 不要提交。 |
