...
最后更新：2026-08-26 15:14（公司。§4.21 T2/T3 人测过。现网当前站 SNIPERS。）。
...
| 分支 | `main` |
| 提交 | 产品 `3c81758`（§4.21 核对只认当前框，T2/T3 人测过）。现网当前站 **SNIPERS**。绿联已归档。 |
...
| 下一台先做 | 家里先 `git pull origin main`。站外核对档案（官网首页 / 发帖入口 / 空字段）已收口，不要重开。下一刀不是代发。不要两边同时改。家里 `localtest.db` 不要提交。 |
