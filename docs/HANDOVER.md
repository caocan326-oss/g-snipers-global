...
最后更新：2026-08-26 19:46（公司。§4.26 同一问再测：博查/Tavily 1→2，结论不变。已切回 SNIPERS。）。
...
| 分支 | `main` |
| 提交 | 产品 `6b5532c`。§4.26 同一问再测博查/Tavily 1→2，提到了、仍没给出官网。已切回 **SNIPERS**。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** 产品未改。`DEMO_LOGIN_ENABLED` 仍关。 |
| 接口实测 | 绿联 100W 抽了两轮，每轮每源 +1。结论两次一样。 |
| 未完成 | `cat_id/3` JSON-LD 受模板限制，不催后台。真发须客户自己的号。不要编 SNIPERS 问句。 |
| 下一台先做 | 家里先 `git pull origin main`。抽查和同一问再测都踩过，不要为额度再抽第三轮。下一刀不是代发。不要两边同时改。 |
