...
最后更新：2026-08-27 12:50（公司。`0f8224d` 信任源地图 + 按轮对照。）。
...
| 分支 | `main` |
| 提交 | `0f8224d` 信任源地图 + 按问句/按轮对照。打开核对人测已过。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `sync-from-local.ps1 -Rebuild`。三处 `0f8224d`。迁移 `025` 已跑。`DEMO_LOGIN_ENABLED` 仍关。 |
| 接口实测 | GEO / 总览应看见信任源地图。SNIPERS 无原句则空着。不要编问句、不要测绿联。 |
