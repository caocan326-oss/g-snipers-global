...
最后更新：2026-08-22 20:23（公司。产品 `5e10164`。绿联身份见 §4.4。同一问三处对齐见 §4.5。客户改法见 §4.6。）。
...
| 分支 | `main` |
| 提交 | 产品 `5e10164`（客户说明留一格给 GEO 改法；改回已写改法时角标回待处理）。绿联身份见 §4.4。三处对齐见 §4.5。客户改法见 §4.6。百炼见 §4.3。运维：§4.1 / §4.2。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。Postgres 仍不映射 5432。 |
| 接口实测 | `test_geo` / `test_customer_brief` 29 通过。发完 health 200。 |
| 未完成 | 没有单独的绿联客户经理号，这次用 `admin@weiyids.com`。改法还没有可复制给微信/邮件的短稿。「客户已上线」还不能留下址。博查源质量仍差。IndexNow / GSC 仍要配钥匙。不要公开注册。不要自动群发。 |
| 下一台先做 | 家里先 `git pull origin main`。强制刷新客户说明：三处里应有 100W 那条。不要两边同时改。家里 `localtest.db` 不要提交。 |
