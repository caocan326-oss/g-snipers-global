...
最后更新：2026-08-24 09:57（公司。产品 `6e09125`。站外一篇稿见 §4.12。不代改见 §4.11。办事卡见 §4.10。）。
...
| C 站内短稿 | **初测 C2 不过**：黄框有结构，但收口只有复查句、无「不代改」。复制 toast 正常。客户说明三处：2 站内 + 1 GEO，说明页有「不代改」。 |
| D 交付口径 | **过** |
...
人测：硬刷新 GEO 100W 卡。不跑新抽查、不改官网、不真发 LinkedIn。看稿两行、复制、打开官方页后关掉、用测试帖链接回填，角标仍待处理。
...
| 分支 | `main` |
| 提交 | 产品 `6e09125`（GEO 卡接站外一篇稿 + 回填帖链接，§4.12）。不代改见 §4.11。办事卡见 §4.10。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `6e09125` 已 `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。Postgres 仍不映射 5432。 |
| 接口实测 | `test_geo` 27 通过。发完 health 200。站外人测待硬刷新。 |
| 未完成 | **人测 §4.12**：100W 卡有英文稿、复制、打开官方页、回填帖链接后仍待处理。A8 同一问再测（有额度再测）。C2「不代改」顺带看一眼。绿联产品页仍须**客户改站**后再测。站外不进交付。IndexNow / GSC 仍要配钥匙。 |
| 下一台先做 | 家里先 `git pull origin main`。硬刷新 GEO 测 §4.12。不要两边同时改。家里 `localtest.db` 不要提交。 |
