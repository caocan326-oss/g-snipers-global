...
最后更新：2026-08-27 10:32（公司。`cat_id/3` JSON-LD 人核过。SNIPERS。）。
...
| 分支 | `main` |
| 提交 | 产品仍 `f0069ca`。交接：`cat_id/3` JSON-LD 人核过。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** 产品未改，只同步交接。`DEMO_LOGIN_ENABLED` 仍关。 |
| 接口实测 | §4.28、§4.29 人测过。`cat_id/3` 标记 10:32 过。 |
| 未完成 | 本机还有未提交的「打开核对」。要上再生产再说。钉着的第 1 张若仍记受模板限制，取消。 |
| 下一台先做 | 海外版家里是 `D:\workspace\G-snipers海外版`，先 `git pull origin main`。不要在 `gsnipers-website` 上改这份产品。不代改、不编问句、不抽第三轮绿联。 |
