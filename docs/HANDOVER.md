...
最后更新：2026-08-23 12:49（家里。产品 `e1d2435`。GEO 工单只跟海外源见 §4.9。错误拆分与控制台横幅见 §4.8。上线地址见 §4.6 `01d8b3b`。绿联身份见 §4.4。同一问三处对齐见 §4.5。客户改法见 §4.6。人工复测修补见 §4.7。）。
...
| 分支 | `main` |
| 提交 | 产品 `e1d2435`（GEO 工单只跟海外源、复测不跟店链，见 §4.9）。分类与错误拆分 `e93dbf6`（§4.8 / §4.9）。上线地址 `01d8b3b`（见 §4.6）。用量与空态 `432fd6a`（见 §4.7）。绿联身份见 §4.4。三处对齐见 §4.5。客户改法见 §4.6。百炼见 §4.3。运维：§4.1 / §4.2。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `e1d2435` 已 `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。Postgres 仍不映射 5432。 |
| 接口实测 | `test_geo_ticket_kinds` 7 通过；`test_geo_citations` 4 通过；`test_http_errors` 通过；开单与博查/Tavily 适配相关用例通过。 |
| 未完成 | 没有单独的绿联客户经理号，这次用 `admin@weiyids.com`。IndexNow / GSC 仍要配钥匙。不要公开注册。不要自动群发。 |
| 下一台先做 | 公司先 `git pull origin main`。强制刷新后：绿联英文问再生成待处理项，只有 Tavily 提到且无官网才开「没给出官网」；只有博查店链不开这张单。不要两边同时改。 |
