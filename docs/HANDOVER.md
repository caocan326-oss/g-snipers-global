...
最后更新：2026-08-26 15:52（公司。产品 `53a3f98`。站内「这周给客户改三处」。现网当前站 SNIPERS。）。
...
| 分支 | `main` |
| 提交 | 产品 `53a3f98`（§4.22 站内这周改三处）。现网当前站 **SNIPERS**。绿联已归档。 |
| 已 push origin / upstream | 否（推完再改是）。 |
| 已发版生产 | **否。** `53a3f98` 待 `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。 |
| 接口实测 | `test_onsite` + `test_customer_brief` + `test_distribution` 46 通过。 |
| 未完成 | 真发须客户自己的号。SNIPERS 没有可填的 LinkedIn / Facebook 公司页。要绿联时从历史网站恢复。 |
| 下一台先做 | 家里先 `git pull origin main`。硬刷新站内看「这周给客户改三处」。不要两边同时改。家里 `localtest.db` 不要提交。 |
