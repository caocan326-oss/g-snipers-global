...
最后更新：2026-08-26 19:40（公司。§4.26 三页客户回告：说明已对上；标题后半截和 JSON-LD 锁在模板里。现网 SNIPERS。）。
...
| 分支 | `main` |
| 提交 | 产品 `6b5532c`。§4.26 绿联抽查已踩实。SNIPERS 三页：说明对上；JSON-LD/标题后半截锁在模板。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** 产品未改。`DEMO_LOGIN_ENABLED` 仍关。 |
| 接口实测 | 绿联 100W：博查/Tavily 各 +1，提到了、没给出官网。三页打开核对与客户回告一致。 |
| 未完成 | `cat_id/3` 标记须改模板，不催后台。真发须客户自己的号。不要编 SNIPERS 问句。 |
| 下一台先做 | 家里先 `git pull origin main`。三页这轮能改的已经改完。JSON-LD 记受模板限制，不要再催同一条。下一刀不是代发。不要两边同时改。 |
