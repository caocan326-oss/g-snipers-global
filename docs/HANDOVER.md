...
最后更新：2026-08-27 10:02（公司。§4.28 人测过。SNIPERS。）。
...
现网 JSON-LD 仍只有 name+url。经理测完可再点一次「记受模板限制」，让这周改能改的三页。等有主题权限的人改完再说一声，再只核 `cat_id/3` 标记。
...
| 分支 | `main` |
| 提交 | 产品仍 `0bd062e`。交接：§4.28 人测过。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** 产品未改，只同步交接。`DEMO_LOGIN_ENABLED` 仍关。 |
| 接口实测 | §4.28 人测过。`cat_id/3` 现网标记仍只有 name+url。 |
| 未完成 | 经理可再点一次「记受模板限制」，这周改能改的三页。等主题改完只核 `cat_id/3` 标记。 |
| 下一台先做 | 家里先 `git pull origin main`。不登 `139.129.99.194`、不代改、不编问句、不抽第三轮绿联。不要把取消后的状态当成没测过。 |
