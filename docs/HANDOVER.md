...
最后更新：2026-08-24 10:29（公司。产品 `9006ec6`。§4.13 人测过。§4.12 人测过。）。
...
| 分支 | `main` |
| 提交 | 产品仍是 `9006ec6`。§4.13 人测过（页面原文过；这台剪贴板读不到，不对粘贴字、不改代码）。§4.12 人测过。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** 产品 `9006ec6` 已 Rebuild。`DEMO_LOGIN_ENABLED` 仍关。Postgres 仍不映射 5432。 |
| 接口实测 | §4.13 人测过。黄框不重复英文稿。清单 297。 |
| 未完成 | 绿联产品页仍须**客户改站**、LinkedIn 须**客户自己发**后再测。A8 同一问再测（有额度再测）。我们不代改、不代发。IndexNow / GSC 仍要配钥匙。 |
| 下一台先做 | 家里先 `git pull origin main`。不要两边同时改。家里 `localtest.db` 不要提交。下一刀不是再改短稿。 |
