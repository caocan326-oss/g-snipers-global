...
最后更新：2026-08-24 10:25（公司。产品 `9006ec6`。客户侧短稿见 §4.13。§4.12 人测过。）。
...
**人测（硬刷新，不跑新抽查、不改官网、不真发）**

1. 客户说明：「发给客户的短稿」里能看到改 usa-65585，以及 `Buyers are asking` / `usa-65585` /「我们不代发」。点「复制给客户」，粘贴结果一样。没有「工作台打勾」。
2. GEO 100W 卡：黄框末行仍是「不代改」，没有整段英文稿。点「复制短稿」，粘贴里才有英文两行。
3. 处理清单条数不变，没有多出站外交付。
...
| 分支 | `main` |
| 提交 | 产品 `9006ec6`（发给客户的短稿带上站外英文稿，§4.13）。§4.12 人测过。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `9006ec6` 已 `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。Postgres 仍不映射 5432。 |
| 接口实测 | `test_geo` + `test_customer_brief` 36 通过。§4.13 人测待硬刷新。 |
| 未完成 | **人测 §4.13**：客户说明「复制给客户」和 GEO「复制短稿」都有英文两行；黄框不重复英文稿；清单条数不变。A8 同一问再测（有额度再测）。绿联产品页仍须**客户改站**后再测。我们不代改、不代发。IndexNow / GSC 仍要配钥匙。 |
| 下一台先做 | 家里先 `git pull origin main`。硬刷新测 §4.13。不要两边同时改。家里 `localtest.db` 不要提交。 |
