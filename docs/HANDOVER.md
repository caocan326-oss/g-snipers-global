...
最后更新：2026-08-26 19:55（公司。§4.26 写清谁改模板、补哪些字段。现网标记仍未补。SNIPERS。）。
...
| 分支 | `main` |
| 提交 | 产品仍 `6b5532c`。交接：`cat_id/3` 再核不过；改主题要客户机 `139.129.99.194` 文件权限，我们不代改。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** 产品未改，只同步交接。`DEMO_LOGIN_ENABLED` 仍关。 |
| 接口实测 | 绿联两轮抽查已踩实。`cat_id/3` 现网标记仍只有 name+url。 |
| 未完成 | 等有 `/template/pc/snipers/` 权限的人改主题。经理只在他们说改完后再核这一页。 |
| 下一台先做 | 家里先 `git pull origin main`。不登 `139.129.99.194`、不代改主题。不挡开发。不编问句、不抽第三轮绿联。 |
