...
最后更新：2026-08-22 14:51（家里。产品 `8a6e0d8`。绿联身份见 §4.4。）。
...
| 分支 | `main` |
| 提交 | 产品 `8a6e0d8`（PDF/GEO 下载失败会提示）。绿联身份见 §4.4。百炼见 §4.3。运维：§4.1 / §4.2。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。Postgres 仍不映射 5432。 |
| 接口实测 | `test_report_export` 4 通过。发完 health 200。 |
| 未完成 | IndexNow / GSC 仍要配钥匙和授权才会亮。生产绿联复测还没走。UptimeRobot 没有原生微信。自动 scp 到第二台云还没有。不要公开注册。不要自动群发。 |
| 下一台先做 | 公司先 `git pull origin main`。不要两边同时改。家里若还有 `backend/localtest.db`，那是本机测试库，不要提交。 |
