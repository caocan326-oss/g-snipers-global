...
最后更新：2026-08-27 23:02（家里。诊断目标灰字不要门锁，随本提交推。）。
...
| 分支 | `main` |
| 产品提交 | 本提交：诊断目标灰字不再用门锁示例。门锁竞品过滤 `82dd632` 已收口。 |
| 文档提交 | 本提交：T3b 收口 + §4.50。 |
| 已 push origin / upstream | 是（随本提交一起推） |
| 已发版生产 | 待本提交 Rebuild。门锁过滤 `82dd632` 已在线上。`DEMO_LOGIN_ENABLED` 仍关。 |
| 本机工作区 | 提交并推完后应干净。 |
| 接口实测 | T3b 过：关键词 `GEO`，保存「竞品新增 0」，刷新无 August Home。灰字刀待硬刷新：清空关键词框（不要保存），应无 smart lock。 |
