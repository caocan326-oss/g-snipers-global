...
最后更新：2026-08-26 19:37（公司。§4.26 绿联 100W 抽查：博查/Tavily 各 +1，提到了、没给出官网。已切回 SNIPERS。）。
...
| 分支 | `main` |
| 提交 | 产品 `6b5532c`。§4.26 绿联 100W：博查 0→1、Tavily 0→1，提到了、没给出官网。已切回 **SNIPERS**。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** 产品未改。`DEMO_LOGIN_ENABLED` 仍关。 |
| 接口实测 | 绿联抽查用量按源 +1。SNIPERS 三页：2、3 过；`cat_id/3` 描述过、JSON-LD 未齐。 |
| 未完成 | `cat_id/3` 还要客户补公司/产品/认证标记。真发须客户自己的号。不要编 SNIPERS 问句。 |
| 下一台先做 | 家里先 `git pull origin main`。抽查记账已踩实，不要重开。第 1 页未齐只回给客户。下一刀不是代发。不要两边同时改。 |
