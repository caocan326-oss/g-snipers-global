...
最后更新：2026-08-27 19:28（门锁补测过已入库。公司本机开发仍暂停。回家 `git pull origin main`。）。
...
| --- | --- |
| 日期 | 2026-08-27 19:28 |
| 最后一台 | **公司** `E:\G-snipers海外版`。今天这台不再改代码。 |
| 下一台 | **家里** `D:\workspace\G-snipers海外版` |
| 分支 | `main` |
| 产品提交 | `1c2ab88` — 门锁拒收先于太短。已发生产。 |
| 文档提交 | 本提交：门锁补测过入库。 |
| 已 push origin / upstream | 是（随本提交一起推） |
| 已发版生产 | **是。** 产品仍是 `1c2ab88`。本轮只同步交接文档，不 Rebuild。 |
| 本机工作区 | 提交并推完后应干净。不要在公司再开功能。 |
| 接口实测 | `c84d173` 走查 1/3/4 过、5 不批。第 2 条补测**过**（19:26）。门锁拒收文案收口。 |
| 记下没挡 | SNIPERS **两份**未批 Fact Pack 草稿，没动。August Home / Level Lock / Qrio 还在诊断目标。 |
...
还没排：批准 Fact Pack（必须客户确认过的英文）、清未批那两份草稿、清诊断目标里的 August/Level/Qrio、Monitor SaaS、代改官网。用户说做哪件再做哪件。
