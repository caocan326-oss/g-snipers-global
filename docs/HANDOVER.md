...
最后更新：2026-08-26 18:58（公司。§4.25 产品 `6b5532c` 已发版，人测未做。现网当前站 SNIPERS。）。
...
- 按钮是「采用已记原句」，不是「生成买家问题」。
- 空着点应 0 条，红字写没有记下的原句 / 搜索词不是问句。买家问题仍空。
- 「买家问题」页有「还没有买家原句。不要编。」
- 客户说明仍写「还没有买家问题，AI 搜索这一块只能写尚未开始。」站内三页不变。
- 没代发、没手加 SNIPERS 问句。
...
| 分支 | `main` |
| 提交 | 产品 `6b5532c`（§4.25 采用已记原句，人测未做）。现网当前站 **SNIPERS**。绿联已归档。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `6b5532c` 已 `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。 |
| 接口实测 | `test_site_identity` + `test_geo` + `test_onsite` + `test_customer_brief` 63 过。 |
| 未完成 | §4.25 等人测。真发须客户自己的号。SNIPERS 没有可填的 LinkedIn / Facebook 公司页。要绿联时从历史网站恢复。 |
| 下一台先做 | 家里先 `git pull origin main`。人测 §4.25（硬刷新 GEO，空着点「采用已记原句」应 0 条）。不要编问句。不要代发。不要两边同时改。家里 `localtest.db` 不要提交。 |
