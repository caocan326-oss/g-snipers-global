...
最后更新：2026-08-24 11:58（公司。产品 `d7f02bf`。站外不说「可发」。§4.14 客户名已修。）。
...
| 分支 | `main` |
| 提交 | 产品 `d7f02bf`（站外不写「可发」，§4.15）。客户名恢复见 §4.14。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `d7f02bf` 已 `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。Postgres 仍不映射 5432。 |
| 接口实测 | `test_distribution` 7 通过。人测待硬刷新看「打开官方发帖页」。 |
| 未完成 | 站外回头一起人测。真发须客户自己的号。A8 有额度再测。绿联页须客户改。IndexNow / GSC 仍要配钥匙。 |
| 下一台先做 | 家里先 `git pull origin main`。硬刷新站外看文案。不要两边同时改。家里 `localtest.db` 不要提交。 |
