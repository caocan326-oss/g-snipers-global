...
最后更新：2026-08-24 14:02（公司。产品 `011c912`。站外可核对公开档案。现网当前站 SNIPERS。）。
...
| 分支 | `main` |
| 提交 | 产品 `011c912`（§4.19 核对公开档案）。现网当前站 **SNIPERS**。绿联已归档。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `011c912` 已 `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。 |
| 接口实测 | `test_distribution` + `test_onsite` 33 通过。档案核对待硬刷新。 |
| 未完成 | 真发须客户自己的号。要绿联时从历史网站恢复。 |
| 下一台先做 | 家里先 `git pull origin main`。硬刷新站外填公开主页再核。不要两边同时改。家里 `localtest.db` 不要提交。 |
