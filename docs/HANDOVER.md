...
最后更新：2026-08-27 12:12（公司。`1bbaee0` 已 Rebuild。）。
...
| 分支 | `main` |
| 提交 | `1bbaee0` 打开核对不再自己勾完；引用资产闭环。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `sync-from-local.ps1 -Rebuild`。三处 `1bbaee0`。迁移 `025` 已跑。`DEMO_LOGIN_ENABLED` 仍关。 |
| 接口实测 | 发版后先点「放回刚拿掉的一页」，再点「打开核对」应新开官网、三处不动。 |
| 未完成 | 没有原句则 GEO 空着。不要编问句。 |
| 下一台先做 | 海外版家里先 `git pull origin main`。总览放回 `cat_id/3`。不代改、不编问句、不抽第三轮绿联。 |
