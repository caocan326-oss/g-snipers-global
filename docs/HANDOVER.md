...
最后更新：2026-08-27（cloud agent。总览这周项 / 打开核对已在仓库，未发版生产。）。
...
官网已经不是 snipers 时，容器启动 seed **不会**再灌门锁演示。它会把客户名改成跟域名走（`www.ugreen.com` → `UGREEN`），并清掉门锁问句/示例。客户名已经是 **SNIPERS** 时，seed 和打开总览 / GEO / 客户说明 / 处理清单也会清掉门锁问句，不会再灌回去；演示客户名仍挂 `snipers.com.cn` 时不动。有抽查记录时必须先删 `geo_sample_results`，否则 backend 起不来（502）。
...
接口：`GET /api/dashboard/workbench` 多 `weekly_onsite`、`weekly_pinned`、`this_week_onsite`、`this_week_open`。Fact Pack 没齐时 `next_actions` 第一条是 `fact-pack`；有三处时后面有 `weekly-three`。

### 4.32 问题板那一百多条不是这周要做完

总览「未关闭 102」会让人觉得应用没交付。那是检查记录加总。这周给客户看的是三处（外加 AI 搜索待处理）。总览第一屏和第三块写这周项；优先队列只列这周三处 + AI 搜索待处理；处理清单改称问题板。接口 `total_open` 仍是加总，`this_week_open` 才是这周项。客户改不改官网不挡。已在仓库。
...
已在仓库。总览可手录原句；搜索词会被拒。作战室「复测到期问句」只抽已记句。
...
已在仓库。常驻复测是现增量里的「同一问再抽」，不是愿景里的全引擎作战室。没有原句不会编，不自动烧配额。
...
已在仓库。总览和站内都有「打开核对 / 放回刚拿掉的一页」。抓页勾成已解决会拨回。
...
| 提交 | 本 PR：总览第一屏/第三块改成这周项；`this_week_open`；打开核对不自勾。交接随本提交。 |
| 已 push origin / upstream | 只推 origin 功能分支。不要当生产已发版。 |
| 已发版生产 | **否。** 不要在服务器 `git pull`。合入后由公司仓 `sync-from-local.ps1 -Rebuild`。 |
| 接口实测 | 工作台 `this_week_open` = 这周三处 + AI 搜索待处理。`total_open` 仍是问题板加总。打开核对只记看过。 |
| 未完成 | 网址过深本周做不到。llms.txt 做不到。贴上了不等于被提到。不要编问句、不要代改、不要再测客户网站。 |
| 下一台先做 | 合入后 `git pull origin main`。对照 `docs/POSITIONING.md`。人测脚本见本 PR。 |
