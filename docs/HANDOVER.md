...
最后更新：2026-08-23 01:22（家里。产品 `526c158`。上线地址见 §4.6 `01d8b3b`。意外失败兜底见 §4.8。绿联身份见 §4.4。同一问三处对齐见 §4.5。客户改法见 §4.6。人工复测修补见 §4.7。）。
...
| 分支 | `main` |
| 提交 | 产品 `526c158`（意外 500 中文兜底，见 §4.8）。上线地址 `01d8b3b`（见 §4.6）。用量与空态 `432fd6a`（见 §4.7）。绿联身份见 §4.4。三处对齐见 §4.5。客户改法见 §4.6。百炼见 §4.3。运维：§4.1 / §4.2。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** 产品 `526c158` / `01d8b3b` 已 `-Rebuild`。本轮文档随后 `sync-from-local.ps1`（不 Rebuild）。`DEMO_LOGIN_ENABLED` 仍关。Postgres 仍不映射 5432。 |
...
| 下一台先做 | 公司先 `git pull origin main`。强制刷新：换站或登录若后端失败，应是「这次没办成，请再试一次」；客户已上线必须填页/帖地址。不要两边同时改。 |
