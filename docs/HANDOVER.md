...
最后更新：2026-08-27 16:54（公司。Fact Pack 总览第一步已发版。）。
...
官网已经不是 snipers 时，容器启动 seed **不会**再灌门锁演示。它会把客户名改成跟域名走（`www.ugreen.com` → `UGREEN`），并清掉门锁问句/示例。客户名已经是 **SNIPERS** 时，seed 和打开总览 / GEO / 客户说明 / 处理清单 / 询盘也会清掉门锁问句和门锁演示询盘（`alex@example.com` / 多门锁），不会再灌回去；演示客户名仍挂 `snipers.com.cn` 时不动。有抽查记录时必须先删 `geo_sample_results`，否则 backend 起不来（502）。
...
现在：客户名已经不是「演示客户 · 智能门锁出海」时，打开总览 / GEO / 客户说明 / 处理清单 / 询盘会清掉门锁买家问题、待处理项、门锁搜索词和门锁演示询盘。SNIPERS 自己的引用材料（含 `snipers.com.cn`）不动。seed 不会再往已改名的 SNIPERS 灌门锁问句或门锁询盘。光写「许可」或「批量安装」不算门锁，避免误伤出口许可 / 紧固件询盘。空的 GEO 是实话，不要编 SNIPERS 买家问题。不要把三句已记原句挂到门锁演示询盘上。不代发。
...
经理不用只进 `/onsite`。总览 `/home` 列出这三处和给客户的短稿。状态：待发给客户 / 已发给客户 / 打开过还没过。已钉住会标出来。可去客户说明、去站内。钉住、发给客户、打开核对仍在站内。空的时候写「这周还没有要改的站内三处」。客户改不改官网不挡交付。我们不代改。

接口：`GET /api/dashboard/workbench` 多 `weekly_onsite`、`weekly_pinned`；有三处时 `next_actions` 第一条是 `weekly-three`。

### 4.32 问题板那一百多条不是这周要做完

总览「未关闭 102」会让人觉得应用没交付。那是检查记录加总。这周给客户看的是三处（外加 AI 搜索待处理）。总览优先队列和第三块改成这周项；处理清单改称问题板。接口 `total_open` 仍是加总，不改数字。客户改不改官网不挡。未发版。
...
生产上点第 1 张 `cat_id/3`「打开核对」：没新开官网，抓页后分析器写成已解决，从这三处拿掉，已解决 9→10，三处变成 `cat_id/1`、`article_id/4`、`article_id/5`，三张变回待发。钉住还在。没改官网。

现改：打开核对 = 打开现网 + 只记看过。总览也有这个按钮。抓页勾成已解决会拨回。可「放回刚拿掉的一页」，`cat_id/3` 回到第 1 张并保持已发给客户。我们不代改。
...
| 分支 | `main` |
| 提交 | 产品 `b2c9bc1`。交接随本提交。 |
| 已 push origin / upstream | 是（随本轮一起推） |
| 已发版生产 | **是。** `sync-from-local.ps1 -Rebuild`。产品 `b2c9bc1`。无新迁移。`DEMO_LOGIN_ENABLED` 仍关。 |
| 接口实测 | 生产 `/api/health` 200。打开总览/询盘会清门锁演示询盘。没 Fact Pack 时总览第一步是去补材料。 |
| 未完成 | 网址过深本周做不到。llms.txt 做不到。贴上了不等于被提到。Fact Pack 要客户已批英文，不要编。不要编问句、不要再抽、不要代改、不要再测客户网站。 |
| 下一台先做 | 海外版家里先 `git pull origin main`。对照 `docs/POSITIONING.md`。总览先补 Fact Pack。 |
