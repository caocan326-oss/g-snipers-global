...
最后更新：2026-08-24 11:49（公司。产品 `b98325b`。历史恢复写回客户名。§4.13 人测过。）。
...
| 分支 | `main` |
| 提交 | 产品 `b98325b`（历史恢复写回客户名，§4.14）。§4.13 人测过。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `b98325b` 已 `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。Postgres 仍不映射 5432。 |
| 接口实测 | 换站隔离测试过。生产已切过 gsnipers 再恢复绿联：工单不串；客户名已补写回。 |
| 未完成 | 站外回头一起人测（对外稿 / LinkedIn 卡 / 执行记录）。真发须客户自己的号。A8 有额度再测。绿联页须客户改。IndexNow / GSC 仍要配钥匙。 |
| 下一台先做 | 家里先 `git pull origin main`。不要两边同时改。家里 `localtest.db` 不要提交。 |
