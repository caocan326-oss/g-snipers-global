...
最后更新：2026-08-27 17:55（公司。this-week overview 已合入 main，准备 Rebuild 发版。）。
...
2026-08-27 人测过：四项都在，绿字「英文周报已复制，可发给工厂老板。」三句原问关单备注都是没提到、没给出官网。询盘这个月 1 条。
...
人测（https://www.weiyids.com 或本机预览，**不改 ugreen.com / snipers.com.cn，不代发，不编问句**）：

1. 总览第一格是「这周改三处」。第三块主数是这周项，不是问题板未关闭。
2. 问题板标题是「问题板」，未关闭可以仍是一百多。
3. 「打开核对」只记看过，卡还在。不要改弹出的客户页。
4. 总览记下 `fastener` 应红字拒。没有真原句就空着。
5. Fact Pack 没齐时「今天先做」第一条是补材料。周报仍可出。
6. SNIPERS 询盘里没有 `alex@example.com`。有真询盘没挂问句时总览会提醒。挂上不是证明被提到。

已在仓库功能分支。**不要当生产已发版。** 合入后公司仓 `sync-from-local.ps1 -Rebuild`。
...
| 分支 | `main`（已合入 this-week overview / PR #1） |
| 提交 | 产品 `45a21f1`。交接随本合并。 |
| 已 push origin / upstream | 合入后推两个远端。 |
| 已发版生产 | 合入后公司仓 `sync-from-local.ps1 -Rebuild`。不要在服务器 `git pull`。 |
...
| 未发版（这轮） | Rebuild 完成前，现网仍是 `b2c9bc1`。 |
| 仍做不到 / 不要做 | 网址过深本周做不到。llms.txt 做不到。贴上了不等于被提到。不要编问句、不要代改、不要再测客户网站、不要登 `139.129.99.194`。 |
| 下一台先做 | `git pull origin main`。对照 `docs/POSITIONING.md` 和 §4.45 人测。 |
