...
| 分支 | `main` |
| 提交 | 产品 `c2ea32a`（站内短稿 + GEO 办事卡，见 §4.10）。此前 `d905cbf` / `363b37b` / `e1d2435`。 |
| 已 push origin / upstream | 是（同 `c2ea32a`）。 |
| 已发版生产 | **是。** `c2ea32a` 已 `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。Postgres 仍不映射 5432。 |
| 接口实测 | 本地 `test_onsite_loop` + `test_customer_brief` + `test_geo` 通过。生产待人登绿联点 GEO 待处理卡与站内复制短稿。 |
| 未完成 | 绿联产品页仍须**客户改站**后再测。站外不进交付。IndexNow / GSC 仍要配钥匙。不要公开注册。不要自动群发。 |
| 下一台先做 | 公司机若有 clone：`git pull origin main`。人测：GEO 待处理卡四样是否够办事；站内短稿能否转发。公司盘本机没有 `E:\G-snipers海外版`。不要两边同时改。 |
