...
最后更新：2026-08-27 16:35（公司。询盘可挂已记问句已发版。）。
...
| 分支 | `main` |
| 提交 | 产品 `4c3e8da`。交接随本提交。 |
| 已 push origin / upstream | 是（随本轮一起推） |
| 已发版生产 | **是。** `sync-from-local.ps1 -Rebuild`。产品 `4c3e8da`。迁移 `026`。`DEMO_LOGIN_ENABLED` 仍关。 |
| 接口实测 | 询盘可挂已记问句。挂上不是证明被提到。英文周报会写出挂了哪几句。 |
| 未完成 | 网址过深本周做不到。llms.txt 做不到。贴上了不等于被提到。现网那 1 条询盘发版后由经理自己挂句。不要编问句、不要再抽、不要代改、不要再测客户网站。 |
| 下一台先做 | 海外版家里先 `git pull origin main`。对照 `docs/POSITIONING.md`。询盘页可挂已记问句。 |
