...
最后更新：2026-08-24 12:11（公司。产品 `f5bdd92`。执行记录回填见 §4.16。§4.15 卡片过。）。
...
| 分支 | `main` |
| 提交 | 产品 `f5bdd92`（执行记录回填不说已提交，§4.16）。§4.15 卡片过。 |
| 已 push origin / upstream | 是。 |
| 已发版生产 | **是。** `f5bdd92` 已 `sync-from-local.ps1 -Rebuild`。`DEMO_LOGIN_ENABLED` 仍关。Postgres 仍不映射 5432。 |
| 接口实测 | `test_distribution` 7 通过。执行记录待硬刷新复看。 |
| 未完成 | **复看执行记录**：角标「已回填」、渠道 LinkedIn Company Page、451 带「登记≠我们代发」。真发须客户自己的号。A8 有额度再测。 |
| 下一台先做 | 家里先 `git pull origin main`。硬刷新站外执行记录。不要两边同时改。家里 `localtest.db` 不要提交。 |
