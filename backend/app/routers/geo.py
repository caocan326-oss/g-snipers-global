...
from app.geo_helpers import CHECKLIST_DEFS, CHECK_STATUSES, ENGINES, OBS_STATUSES, build_llms_txt
from app.models import DemandSignal, GeoAsset, GeoChecklistItem, GeoObservation, GeoPrompt, SeoPage, Tenant, User
...
def _prompt_out(row: GeoPrompt) -> GeoPromptOut:
    return GeoPromptOut(
        ...
        observations=[GeoObservationOut.model_validate(o, from_attributes=True) for o in row.observations],
    )
...
def create_prompt(
    ...
    row = GeoPrompt(tenant_id=user.tenant_id, **body.model_dump())
    db.add(row)
    db.flush()
